...
class Abstract {
  ...
  buildShaders() {
    ...
    const tokens = {
      vert: tokenize__default["default"](this.raw.vertex),
      frag: tokenize__default["default"](this.raw.fragment)
    };
    const descoped = {
  ...
  getHash() {
    ...
    return hash__default["default"](values);
  }
...
Depth.u_near = 2;
Depth.u_far = 10;
Depth.u_origin = new THREE.Vector3(0, 0, 0);
...
Noise.u_alpha = 1;
Noise.u_scale = 1;
Noise.u_offset = new THREE.Vector3(0, 0, 0);
Noise.vertexShader = `
    varying vec3 v_position;
...
Displace.u_scale = 1;
Displace.u_offset = new THREE.Vector3(0, 0, 0);
...
Normal.u_alpha = 1;
Normal.u_direction = new THREE.Vector3(1, 1, 1);
Normal.vertexShader = `   
...
class LayerMaterial extends CustomShaderMaterial__default["default"] {
  constructor({
    color,
    alpha,
    lighting,
...
class LaminaLoader extends THREE__namespace.Loader {
  constructor(manager) {
    super(manager);
    this.texLoader = new THREE__namespace.TextureLoader();
  }
        ...
        const layers = await Promise.all(data.layers.map(async layer => {
          ...
          await Promise.all(Object.entries(layer.currents).map(async ([key, val]) => {
            if (typeof val === 'string' && isBase64UrlImage(val)) {
              ...
              t.encoding = THREE__namespace.sRGBEncoding; //@ts-ignore
...
async function downloadObjectAsJson(exportObj, exportName) {
  ...
  if (obj.metadata.type === 'mat') {
    const o = obj;
    await Promise.all(o.layers.map(async layer => {
      await Promise.all(Object.entries(layer.currents).map(async ([key, val]) => {
    ...
    const o = obj;
    await Promise.all(Object.entries(o.base.currents).map(async ([key, val]) => {
      if (isValidHttpUrl(val)) {
...
function useExports(ref, store) {
  leva.useControls({
    ...
    Export: leva.button(() => {
      ...
      downloadObjectAsJson(file, (_serializedBase$curre = serializedBase.currents) == null ? void 0 : _serializedBase$curre.name);
    })
  }, {
    store
  });
...
function useAttach(ref, store) {
  React__namespace.useLayoutEffect(() => {
    ...
    if (root) {
      ...
      levaRoot.render( /*#__PURE__*/React__namespace.createElement(leva.LevaPanel, {
        titleBar: {
    ...
    if ((_ref$current = ref.current) != null && _ref$current.__lamina__debuggerNeedsUpdate) {
      ...
      serializedLayers.forEach((layer, layerIndex) => {
        ...
        Object.entries(layer.currents).forEach(([k, val]) => {
          if (!layers[layerIndex].debugExclude.includes(k)) {
...
const LaminaDebugger = /*#__PURE__*/React__namespace.forwardRef(({
  ...
  if (!Array.isArray(children)) {
    return /*#__PURE__*/React__namespace.createElement(React__namespace.Fragment, null, /*#__PURE__*/React__namespace.cloneElement(children, {
      ref: mergeRefs__default["default"]([forwardRef, ref])
    }));
  } else {
...
exports.LaminaDebugger = LaminaDebugger;
